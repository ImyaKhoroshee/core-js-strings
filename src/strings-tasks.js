...
function getStringLength(value) {
  return value === undefined || value === null || undefined ? 0 : value.length;
}
...
function countVowels(str) {
  ...
  return str.split('').filter((char) => dictionary.includes(char)).length;
}
...
function isPalindrome(str) {
  const pureString = str.replace(/[^A-Za-z]/g, '').toLowerCase();
  return pureString === pureString.split('').reverse().join('');
}
